...
@router.post("/start")
async def start_stream(orchestrator=Depends(get_orchestrator)) -> dict:
    await orchestrator.start_stream()
    return {"status": "ok", "message": "stream started"}


@router.post("/stop")
async def stop_stream(orchestrator=Depends(get_orchestrator)) -> dict:
    await orchestrator.stop_stream()
    return {"status": "ok", "message": "stream stopped"}


@router.get("/status")
async def stream_status(orchestrator=Depends(get_orchestrator)) -> dict:
    return orchestrator.stream_status().model_dump(mode="json")


@router.get("/latest-frame")
async def latest_frame(orchestrator=Depends(get_orchestrator)):
...
@router.get("/mjpeg")
async def stream_mjpeg(orchestrator=Depends(get_orchestrator)):
