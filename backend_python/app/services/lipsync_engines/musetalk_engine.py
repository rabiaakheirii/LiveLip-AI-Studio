...
class MuseTalkEngine(BaseLipSyncEngine):
    ...
    async def process_frame(self, frame: FramePacket, audio_bytes: bytes | None = None) -> EngineFrameResult:  # noqa: ARG002
        if not self._enabled:
            raise RuntimeError("MuseTalk engine disabled (set ENABLE_EXPERIMENTAL_ENGINES=true)")
        raise NotImplementedError("TODO: integrate MuseTalk inference worker")

    async def render_chunk(self, audio_path: Path, frame_path: Path, output_dir: Path, source: str = "webcam") -> PreviewChunk:  # noqa: ARG002
        if not self._enabled:
            raise RuntimeError("MuseTalk engine disabled")
        raise NotImplementedError("TODO: integrate MuseTalk chunk rendering")
